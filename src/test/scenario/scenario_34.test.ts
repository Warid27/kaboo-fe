...
describe('Scenario 34: Bot Tap Timing (Reaction Speed)', () => {
  ...
  test('Hard bot should tap very quickly (0.2-0.8s)', async () => {
    ...
    expect(useOfflineStore.getState().tapState?.phase).toBe('window');
    ...
    vi.advanceTimersByTime(1000); // Increased from 500 to 1000 to cover full hard bot delay range
    expect(useOfflineStore.getState().tapState?.phase).toBe('swapping');
    expect(useOfflineStore.getState().players[1].cards.length).toBe(0); // Card was removed from bot
  });
  ...
  test('Easy bot should tap slowly (2-3s)', async () => {
    ...
    const phase = useOfflineStore.getState().tapState?.phase;
