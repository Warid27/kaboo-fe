...
import { TIMER_OPTIONS, TARGET_SCORE_OPTIONS, PLAYER_COUNTS, DIFFICULTY_OPTIONS } from './constants';
...
export function OfflineView({ settings, updateSettings }: OfflineViewProps) {
  ...
  return (
    ...
    >
      ...
      {/* Bot Difficulty (Specific to Offline) */}
      <div>
        <label className="mb-2 block font-display text-sm font-semibold text-muted-foreground">
          Bot Difficulty
        </label>
        <div className="flex gap-2">
          {DIFFICULTY_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => updateSettings({ botDifficulty: opt.value })}
              className={`flex-1 rounded-lg px-3 py-2 font-display text-sm font-bold transition-all ${
                settings.botDifficulty === opt.value
                  ? 'gradient-accent text-accent-foreground glow-accent'
                  : 'bg-muted text-muted-foreground hover:bg-muted/80'
              }`}
            >
